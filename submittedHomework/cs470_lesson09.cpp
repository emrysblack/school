...
*
*    Now it has a basic access control scheme implemented
************************************************************************/
...
{
   { "/home/cs470/lesson09/sam.txt" },
   { "/home/cs470/lesson09/sue.txt" },
   { "/home/cs470/lesson09/sly.txt" }
};
...
private:
   ...
   bool  hasAccess(int iScore); // is this not the resource level?
};
...
{
   assert(iScore >= 0 && iScore < scores.size());
   return(hasAccess(iScore) ? scores[iScore].score : -1.0);
}
...
{
   assert(iScore >= 0 && iScore < scores.size());
   return(hasAccess(iScore) ? scores[iScore].weight : -1.0);
}
 ...
 * GET ACCESS
 **********************************************/
bool StudentGrade::hasAccess(int iScore)
{
   assert(iScore >= 0 && iScore < scores.size());

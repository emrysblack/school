...
void displayTable(int numDays, int offset)
{
   ...
   for (offsetCount; counter <= 0 && offsetCount < 6; counter++) // place offsets
      cout << "    ";
   while (day <= numDays) // place date numbers
   {
      if (columnCounter++ % 7 == 0) // check if new line needed after next entry
      {
         cout << setw(4)
              << day
              << "\n";
         columnCounter = 1; // reset column counter for beggining of week
      }
      else
      {
         cout << setw(4)
